#!/usr/bin/env python
"""
Analyze Insider advisor performance by session to verify correlation with:
1. Number of discoveries per session
2. High volume trading days (using SPY volume as proxy)

Usage:
    python analyze_insider_by_session.py
    python analyze_insider_by_session.py --days 30
"""
import os
import sys
import django
...
def analyze_insider_by_session(days=60):
    """Analyze Insider discoveries grouped by session."""
    ...
    print(f'\nAnalyzing Insider discoveries from last {days} days')
    print(f'Found {sessions.count()} sessions')
    print(f'Found {discoveries.count()} Insider discoveries\n')
    ...
    # Display results
    print('=' * 120)
    print('INSIDER PERFORMANCE BY SESSION (End of Day Evaluation)')
    print('=' * 120)
    ...
    # Show top performing sessions (only with valid data)
    print('\n' + '=' * 120)
...
if __name__ == '__main__':
    ...
    analyze_insider_by_session(args.days)
