"""
Test 4-ducks LLM: grade EX-99.1 earnings filing on past_performance, future_performance, expectation_gap,
and market_sentiment_score / market_reaction.
...
# Default when no market context supplied (same structure as real context for parsing)
DEFAULT_MARKET_CONTEXT = """Sector: Not provided
52-Week Range: Not provided - Not provided (Current Price: Not provided)
Relative Strength: Not provided
Market Sentiment: Not provided"""
...
Given an earnings press release (EX-99.1 from an 8-K), evaluate and grade the following four metrics independently.
Use only the information in the document for metrics 1–3. For metric 4, also use the provided Market Context if supplied.
Ignore stock price movement in the document for metrics 1–3.
Ignore analyst consensus unless explicitly mentioned in the text.
Do not speculate beyond the text (and, for metric 4, the Market Context).

TASK
Grade the following four metrics:
...
Predict the short-term (1-3 day) share price direction. Synthesize the 8-K data with the provided Market Context.
Consider Mean Reversion: If the stock is at a 52-week high, is the "good news" already priced in?
Consider Sector Beta: How does this news land given current sector headwinds/tailwinds?
Consider "Whisper Numbers": Does the tone suggest a beat-and-raise that offsets a trailing miss?
Market Context is supplied below in this format: Sector; 52-Week Range (Current Price); Relative Strength; Market Sentiment.
...
For metric 4, if Market Context is not provided, set market_sentiment_score to 0 and justify in market_reaction.rationale that no context was supplied.
...

{
  ...
  "justifications": {
    ...
    "market_reaction": {
      ...
      "rationale": "Considering the 52-week high and the sector's current volatility, this positive EPS beat may face 'sell the news' pressure."
    }
  }
}

Replace <integer> with -2, -1, 0, +1, or +2. Replace the placeholder strings with brief justifications (1–2 sentences per metric). For market_reaction, set predicted_direction to Bullish, Bearish, or Neutral and provide a short rationale.
...
----------------------------------------
MARKET CONTEXT (use for Market Reaction Prediction only)
Format: Sector; 52-Week Range (Current Price); Relative Strength; Market Sentiment.
----------------------------------------
...
def run(path: Path | None = None):
    ...
    prompt = prompt.replace("<<<MARKET_CONTEXT>>>", DEFAULT_MARKET_CONTEXT)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Grade EX-99.1 on 4 dimensions via Gemini (3 ducks + market sentiment)")
