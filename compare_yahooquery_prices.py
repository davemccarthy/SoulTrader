#!/usr/bin/env python
"""
Compare yesterday's yahooquery screener results with today's prices
to calculate average gain/loss.
"""
...
def load_yesterday_snapshot(filename):
    """Load yesterday's stock snapshot."""
    stocks = []
...
def fetch_today_prices(stocks):
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        print(f"  Fetching batch {i//batch_size + 1} ({len(batch)} stocks)...")
            ...
            for symbol in batch:
                try:
                    ...
                    if today_price:
                        ...
                        print(f"    Warning: No price found for {symbol}")
    ...
    # Create lookup dict
    price_dict = {r['symbol']: r['today_price'] for r in results}
...
def analyze_results(stocks):
    ...
    print("\n" + "="*80)
    print("YAHOOQUERY PRICE COMPARISON RESULTS")
    print("="*80)
    print(f"\nTotal stocks analyzed: {len(valid_stocks)}")
    print(f"Stocks with price data: {len(valid_stocks)}")
    print(f"Stocks without price data: {len(stocks) - len(valid_stocks)}")
    ...
    for i, stock in enumerate(sorted_stocks[:10], 1):
        print(f"  {i:2d}. {stock['symbol']:<6} {stock['name'][:40]:<40} "
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python compare_yahooquery_prices.py <snapshot_file>")
        print("Example: python compare_yahooquery_prices.py yahooquery_stocks_20251119_074225.txt")
        sys.exit(1)
    
    snapshot_file = sys.argv[1]
    
    print(f"Loading snapshot from: {snapshot_file}")
    stocks = load_yesterday_snapshot(snapshot_file)
    print(f"Loaded {len(stocks)} stocks from snapshot")
