...
from core.models import Discovery, Stock, Trade
...
def fifo_outcomes_for_user_stock(
    ...
    for t in buy_trades:
        ...
        pct = float((exit_value - cost) / cost * 100) if cost and cost > 0 else 0.0
        outcomes.append(
            LotOutcome(
                advisor=advisor,
                cost=cost,
...
def peak_holdings_cost(user_ids: list[int], cutoff) -> float:
    ...
    for t in trades:
        ...
        total_cost = sum(
            ...
            for (_, _), lots in positions.items()
            for sh, cost in lots
        )
        if total_cost > peak:
            peak = total_cost
...
def print_scoreboard(scoreboard: dict, stdout, cols: tuple = COLUMNS) -> None:
