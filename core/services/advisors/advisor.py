...
class AdvisorBase:
    ...
    def get_last_trading_day(test_date=None):
        ...
        return financial_polygon.get_last_trading_day(test_date=test_date)

    @classmethod
    def _fetch_polygon_stocks_for_date(cls, reference_date):
        """Compatibility wrapper for financial polygon service."""
        return financial_polygon._fetch_polygon_stocks_for_date(reference_date)

    @classmethod
    def get_filtered_stocks(cls, sa=None, min_price=None, max_price=None, min_volume=None, test_date=None):
        """Compatibility wrapper for financial polygon service."""
        return financial_polygon.get_filtered_stocks(
            min_price=min_price,
            max_price=max_price,
    ...
    def discovered(self, sa, symbol, explanation, sell_instructions = None, weight = 1.0, health = None):
        ...
        if sell_instructions:
            ...
            for instruction_type, instruction_value, value2 in sell_instructions:
                ...
                if instruction_type == 'TARGET_PERCENTAGE':
                    # Multiplier vs average price (e.g. 1.02 = +2%); evaluated at sell-check time.
                    instruction.value1 = Decimal(str(instruction_value)) if instruction_value is not None else None
                elif instruction_type in ['STOP_PERCENTAGE', 'END_DAY', 'PERCENTAGE_DIMINISHING', 'PERCENTAGE_AUGMENTING']:
...
"""
    ...
    def ask_gemini(self, prompt, timeout=120.0, use_search=False):
        ...
        https://ai.google.dev/gemini-api/docs/rate-limits
        """
        model, results, next_model_idx, next_key_idx = llm_ask_gemini(
            prompt=prompt,
