...
class Edgar(AdvisorBase):
    ...
    def compute_filter3_pass(self, filing, verbose: bool = False) -> bool:
        ...
        if price is not None and isinstance(price, (int, float)):
            ...
            if price > FILTER3_PRICE_MAX:
                ...
                return False
        # Profitability: flag warning only (not a hard fail)
        profit_margin = info.get("profitMargins")
        if profit_margin is None or (isinstance(profit_margin, (int, float)) and profit_margin <= 0):
            comments.append("⚠️ BASIC FILTER WARNING: Company not profitable (negative or missing profit margin)")
            logger.warning(f"FILTER3: {ticker or 'N/A'} not profitable (profitMargins={profit_margin})")
    ...
    def compute_filter6_pass(self, filing, verbose: bool = False):
        ...
        try:
            ...
            if reported_val != 0:
                # Use abs to avoid complex score when reported EPS is negative (e.g. loss vs smaller loss)
                eps_score = min(surprise_val, 50) * (abs(reported_val) ** 0.5)
                if verbose:
                    comments.append(f"AV EPS surprise {surprise_str}% reported {reported_str} score {eps_score}")
                return surprise_val > 0, comments
        ...
        if actual_eps is not None and estimated_eps is not None and estimated_eps != 0:
            ...
            # Use abs to avoid complex score when actual_eps is negative
            eps_score = min(surprise_pct, 50) * (abs(actual_eps) ** 0.5)
            if verbose:
                comments.append(f"8-K actual ${actual_eps:.2f} ({source}) + AV estimate ${estimated_eps:.2f} → surprise {surprise_pct:+.2f}% score {eps_score}")
            return surprise_pct > 0, comments
    ...
    def compute_filter7_pass(self, filing, verbose: bool = False):
        ...
        Strict pass: past_performance >= 0, future_performance >= 1, expectation_gap >= 0.
        Only future < 1 and (past or expectation_gap) < 0 are hard fails.
        Returns True (pass), False (fail), or None (no text / LLM error / inconclusive).
        TODO: Interpret counts-based scoring (positive/negative) when THREE_DUCKS_PROMPT is updated; see test_3ducks_llm.
        """
        text = _get_ex99_text(filing)
        comments = []
        ...
        if past is None or future is None or gap is None:
            ...
            return None,[]
        # Hard fail: past < 0, future < 1, or expectation_gap < 0 (allow gap == 0)
        if past < 0 or future < 1 or gap < 0:
            return False,[]
    ...
    def analyze_8k_advanced(self, filing, meta_requirements: dict, verbose: bool = False):
        ...
        ticker = getattr(filing, "ticker", None) or cik_to_ticker(cik)
        advanced_comments = []
        f6_result = meta_requirements.get("filter6")

        # Filter7 first (3-ducks): can hard-fail; avoids running filter6/filter8 when doc fails
        if meta_requirements.get("filter7") is None:
            ...
            if f7_pass is False or f7_pass is None:
                ...
                return f7_pass, (f7_pass, f6_result, None), advanced_comments
            advanced_comments.extend(f7_comments or [])

        # Filter6 (EPS surprise): if no surprise data (None), continue to filter8; only False is hard fail
        if meta_requirements.get("filter6") is None:
            ...
            if f6_pass is False:
                logger.info(f"compute_filter6_pass(EPS beat): ticker={ticker or 'N/A'}, returns False (miss)")
                return f6_pass, (True, f6_pass, None), (advanced_comments + (f6_comments or []))
            if f6_pass is None:
                logger.info(f"compute_filter6_pass(EPS surprise): ticker={ticker or 'N/A'}, no surprise data → continue to filter8")
        ...
        if meta_requirements.get("filter8") is None:
            ...
            if f8_pass is False or f8_pass is None:
                ...
                return f8_pass, (True, f6_result, f8_pass), advanced_comments
        ...
        return True, (True, f6_result, True), advanced_comments
    ...
    def _meta_filters_complete(self, meta):
        """True if filter7 and filter8 are set (non-None). filter6 may be None when no EPS surprise data."""
        m = meta or {}
        return (
            m.get("filter7") is not None
