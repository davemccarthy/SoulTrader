"""
Vulture overlay advisor.

Phase-1 design - intraday:
...
class Vulture(AdvisorBase):
    ...
    def discover(self, sa):
        ...
        if len(symbols) < MIN_UNIVERSE_SIZE:
            ...
            return
        logger.info("Intraday universe: %s held symbols (enabled funds)", len(symbols))

        returns: Dict[str, float] = {}
        missing_return_symbols: List[str] = []
        for symbol in symbols:
            ret = _intraday_return_since_open(symbol)
            if ret is None:
                missing_return_symbols.append(symbol)
                continue
            returns[symbol] = ret
        ...
        if len(returns) < MIN_UNIVERSE_SIZE:
            ...
            if missing_return_symbols:
                logger.info("Intraday missing intraday data for: %s", ", ".join(missing_return_symbols[:25]))
            return
        ...
        weakest = ", ".join(f"{sym}:{ret:.2f}%" for sym, ret in sorted_returns[:8])
        strongest = ", ".join(f"{sym}:{ret:.2f}%" for sym, ret in sorted_returns[-5:])
        logger.info(
            "Intraday spread snapshot: size=%s red=%s red_ratio=%.2f median=%.2f%%",
            len(returns),
            red_count,
            red_ratio,
            median_ret,
        )
        logger.info("Intraday weakest symbols: %s", weakest)
        logger.info("Intraday strongest symbols: %s", strongest)
        ...
        candidates = []
        extreme_breakdown_symbols: List[str] = []
        for symbol, ret in returns.items():
            if ret < EXTREME_BREAKDOWN_CUTOFF:
                extreme_breakdown_symbols.append(symbol)
                continue
            if ENTRY_DROP_MIN <= ret <= ENTRY_DROP_MAX:
                candidates.append((symbol, ret))
        logger.info(
            "Intraday candidate pass: %s symbols in entry band [%.2f%%, %.2f%%]; %s extreme breakdown exclusions (< %.2f%%)",
            len(candidates),
            ENTRY_DROP_MIN,
            ENTRY_DROP_MAX,
            len(extreme_breakdown_symbols),
            EXTREME_BREAKDOWN_CUTOFF,
        )
        if extreme_breakdown_symbols:
            logger.info("Intraday excluded breakdown symbols: %s", ", ".join(extreme_breakdown_symbols[:20]))

        discoveries = 0
        skipped_recent = 0
        skipped_stock_create = 0
        skipped_health_missing = 0
        for symbol, ret in sorted(candidates, key=lambda x: x[1]):
            ...
            discoveries += 1
            logger.info("Intraday discovered %s at return %.2f%%", symbol, ret)
        ...
        logger.info(
            "Intraday processed %s: discoveries=%s candidates=%s universe=%s red_ratio=%.2f median=%.2f%% skipped_recent=%s skipped_stock=%s skipped_health=%s",
            today,
            discoveries,
            len(candidates),
            len(returns),
            ...
            median_ret,
            skipped_recent,
            skipped_stock_create,
            skipped_health_missing,
        )
