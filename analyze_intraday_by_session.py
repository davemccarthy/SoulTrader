#!/usr/bin/env python
"""
Analyze Intraday advisor performance by session to verify correlation with:
1. Number of discoveries per session
2. High volume trading days (using SPY volume as proxy)

Usage:
    python analyze_intraday_by_session.py
    python analyze_intraday_by_session.py --days 30
"""
import os
import sys
import django
...
def analyze_intraday_by_session(days=60):
    """Analyze Intraday discoveries grouped by session."""
    ...
    print(f'\nAnalyzing Intraday discoveries from last {days} days')
    print(f'Found {sessions.count()} sessions')
    print(f'Found {discoveries.count()} Intraday discoveries\n')
    ...
    for session_id, data in session_data.items():
        ...
        for trade in buy_trades:
            ...
            # Use EOD price from the trade date (when we actually bought)
    ...
    # Display results
    print('=' * 120)
    print('INTRADAY PERFORMANCE BY SESSION (End of Day Evaluation)')
    print('=' * 120)
    ...
    # Filter out sessions with no valid data (0/0 gainers/losers)
    ...
    count_ranges = {
        ...
        '6-10 discoveries': [p for p in sessions_with_data if 6 <= p['discovery_count'] <= 10],
        '11+ discoveries': [p for p in sessions_with_data if p['discovery_count'] >= 11],
    }
    ...
    # Analyze by market volume (only sessions with data)
    ...
    print('\n' + '=' * 120)
    print('TOP PERFORMING SESSIONS (With Valid Data)')
    print('=' * 120)
    
    top_by_winrate = sorted(sessions_with_data, key=lambda x: (x['win_rate'], x['gainers'] + x['losers']), reverse=True)[:10]
    print('\nTop 10 by Win Rate:')
    for perf in top_by_winrate:
        print(f"  SA#{perf['session_id']} ({perf['date']}): {perf['discovery_count']} discoveries, "
              f"{perf['gainers']}/{perf['losers']} G/L, {perf['win_rate']:.1f}% win rate, {perf['avg_change']:+.2f}% avg")
    
    top_by_change = sorted(sessions_with_data, key=lambda x: x['avg_change'], reverse=True)[:10]
    print('\nTop 10 by Average % Change:')
    for perf in top_by_change:
        print(f"  SA#{perf['session_id']} ({perf['date']}): {perf['discovery_count']} discoveries, "
              f"{perf['gainers']}/{perf['losers']} G/L, {perf['win_rate']:.1f}% win rate, {perf['avg_change']:+.2f}% avg")
